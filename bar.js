...
let mouseover3 = function (d) {
    let color_span = `<span style="color: ${color3(d.State_Code)}; font-size: 18px; font-weight:bold;">`;
    let html = `${d.State_Code}<br/>
                Tweets Count:<br/>
                ${color_span}${d.Tweets_Cnt}</span>`;
    // console.log(color3(d.State_Code));
    ...
    tooltip3.html(html)
        ...
        .style("box-shadow", `3px 3px 7px ${color3(d.State_Code)}`)
    ...
    svg3.select(`#rect-${d.State_Code}`).attr("fill", function (d) {
        return darkenColor(color3(d.State_Code), 0.5);
    });
}
...
d3.csv("./data/full_dataset.csv").then(function (data3) {
    ...
    // y-label
    y3_desc.attr("transform", `translate(-60, ${(height - margin.top - margin.bottom) / 2 + 80})rotate(-90)`)       // HINT: Place this at the center left edge of the graph - use translate(x, y) that we discussed earlier
        .style("text-anchor", "middle")
        .style("font-size", 18)
        .text("Tweets Counts in Each State");

    // Add chart title
    x3_desc.attr("transform", `translate(500, -25)`)      // HINT: Place this at the top middle edge of the graph - use translate(x, y) that we discussed earlier
        .style("text-anchor", "middle")
        .style("font-size", 18)
        .text("Tweets Count VS State Code");

    title3.style("font-size", 18)
    ...
    var bar3 = svg3.selectAll("rect")
        ...
        .attr("fill", function (d) { return color3(d.State_Code); })
